...
description: "Guidelines and skill for building reusable UI components following the PulsePass design system"
---
# Component Builder — Design System Rules

## Instructions
1. Always build entirely custom elements using standard semantic HTML and React Functional Components. Do NOT use Shadcn UI.
2. Store generic components in the appropriate `components/` directory.
3. Ensure accessibility aligns with WCAG 2.2 AA.
4. Export components explicitly and use TypeScript interfaces/types for props.

---

## Token Files Are the Source of Truth
The project has one design token file. The agent must never modify them:
`theme-tokens.css` — all color values, all font sizes, weights, line heights, and font families.
...
## Mobile-First
PulsePass users are primarily on mobile. Every component must be built mobile-first:
